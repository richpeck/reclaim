  ...
  ##########################################
    ...
    if files.any?
      ...
      Meta::News.all.each do |news|
        ...
        news.featured_image.attach(io: File.open(file), filename: File.basename(file), content_type: 'image/jpeg', identify: false) unless news.featured_image.attached? # => Upload stored files
